...
js_create_object_with_properties(js_env_t *env, js_value_t *prototype, js_value_t *const property_names[], js_value_t *const property_values[], size_t property_count, js_value_t **result) {
  ...
  for (size_t i = 0; i < property_count; i++) {
    jerry_value_free(jerry_object_set(object, js__value_from_abi(property_names[i]), js__value_from_abi(property_values[i])));
  }
...
js__set_integrity_level(js_env_t *env, js_value_t *object, const char *method) {
  ...
  jerry_value_t constructor = jerry_object_get_sz(global, "Object");

  jerry_value_free(global);

  if (jerry_value_is_exception(constructor)) {
    js__uncaught_exception(env, constructor);

    return js__error(env);
  }

  jerry_value_t function = jerry_object_get_sz(constructor, method);

  jerry_value_free(constructor);

  if (jerry_value_is_exception(function)) {
    js__uncaught_exception(env, function);

    return js__error(env);
  }

  jerry_value_t argv[1] = {js__value_from_abi(object)};

  env->depth++;

  jerry_value_t value = jerry_call(function, jerry_undefined(), argv, 1);
...
js__threadsafe_function_finalize(js_threadsafe_function_t *function) {
  ...
  jerry_value_free(function->function);
...
js__threadsafe_function_call(js_threadsafe_function_t *function) {
  ...
  if (js__threadsafe_queue_pop(&function->queue, &data)) {
    ...
    if (function->cb) {
      function->cb(env, js__value_to_abi(function->function), function->context, data);
    } else {
      js_value_t *receiver;
      err = js_get_undefined(env, &receiver);
      assert(err == 0);

      err = js_call_function(env, receiver, js__value_to_abi(function->function), 0, NULL, NULL);
      (void) err;
    }
...
js_create_threadsafe_function(js_env_t *env, js_value_t *function, size_t queue_limit, size_t initial_thread_count, js_finalize_cb finalize_cb, void *finalize_hint, void *context, js_threadsafe_function_cb cb, js_threadsafe_function_t **result) {
  ...
  if (function) {
    threadsafe_function->function = jerry_value_copy(js__value_from_abi(function));
  } else {
    threadsafe_function->function = jerry_null();
  }
